...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth){
   ...
   let count = 0;
   let totalSavings = 0;
   let totalPercentLossByMonth = percentLossByMonth;
   
   while (startPriceNew >= startPriceOld + totalSavings) {
     count++;
     
     if(count % 2 === 0) {
       totalPercentLossByMonth += 0.5;
     }
     
     startPriceOld -= (totalPercentLossByMonth/100) * startPriceOld;
     startPriceNew -= (totalPercentLossByMonth/100) * startPriceNew;
     totalSavings += savingperMonth;
   }
   
   return [count, Math.round(startPriceOld + totalSavings - startPriceNew)]
 }