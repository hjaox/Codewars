...
function chooseBestSum(t, k, ls) {
    const possibleChoices = ls.reduce((arr, curr) => {
      return arr.concat(arr.filter(choice => choice.length < k).map(choice => choice.concat([curr])))
    },[[]]);
    
    const filteredChoices = possibleChoices.filter(item => item.length === k && item.reduce((sum, curr) => sum += curr, 0) <= t);
    
    if(!filteredChoices.length) {
      return null
    }
    return Math.max(...filteredChoices.map(choice => choice.reduce((sum, num) => sum += num, 0)))
  }