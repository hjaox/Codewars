/*
https://www.codewars.com/kata/5a51717fa7ca4d133f001fdf/javascript
...
function packBagpack(scores, weights, capacity) {
    let scoresByWeight = Array.from({ length : capacity + 1}, _ => 0);
    
    for(let i = 0; i < weights.length; i++) {
      scoresByWeight = scoresByWeight
      .map((score, weight) => Math.max(score, weights[i] <= weight && scoresByWeight[weight - weights[i]] + scores[i]));
    }
    
    return scoresByWeight.pop();
  }